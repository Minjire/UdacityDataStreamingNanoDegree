...
spark = SparkSession.builder.appName("stedi-app-join").getOrCreate()
...
redisServerDF.withColumn("value", from_json("value", redisMessageSchema)).select(col('value.*')).createOrReplaceTempView("RedisSortedSet")
...
zSetEntriesDecodedDF.withColumn("customer", from_json("encodedCustomer", customerJSONSchema)) \
    .select(col('customer.*')) \
...
customerRiskDF.withColumn("value", from_json("value", customerRiskJSONSchema)).select(col('value.*')).createOrReplaceTempView("CustomerRisk")
...
joinedCustomerDF = customerRiskStreamingDF.join(emailAndBirthYearStreamingDF, expr("""
    customer = email
"""))
...
# joinedCustomerDF.writeStream.outputMode("append").format("console").start().awaitTermination()

joinedCustomerDF.selectExpr("cast(customer as string) as key", "to_json(struct(*)) as value")\
    .writeStream \
    .format("kafka") \
    .option("kafka.bootstrap.servers", "localhost:9092") \
